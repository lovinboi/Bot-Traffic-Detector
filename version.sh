...
# Get storage location for the new LXC's disk
...
# Find a storage location that accepts VZTMPL content
TEMPLATE_STORAGE=""
for storage in $(pvesh get /storage --output-format yaml | grep "storage:" | awk '{print $2}'); do
...
if ! pveam list ${TEMPLATE_STORAGE} | grep -q $TEMPLATE_NAME; then
    ...
    pveam download ${TEMPLATE_STORAGE} $TEMPLATE_NAME >/dev/null || msg_error "Failed to download Debian 12 template. Check if storage '${TEMPLATE_STORAGE}' is online and has space."
fi
