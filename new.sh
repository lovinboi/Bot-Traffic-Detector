...
cd /opt
if ! git clone https://github.com/open-webui/open-webui.git; then
    msg_error "The 'git clone' command failed. Please check your network connection and that git is installed."
fi
# A sync command to help ensure all files are written to disk before proceeding.
sync
if [ ! -f "/opt/open-webui/backend/main.py" ]; then
    msg_error "Repository was cloned, but is incomplete (missing main.py). Please check for network issues or disk space."
fi
