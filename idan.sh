...
if pct status $CT_ID >/dev/null 2>&1; then
    msg_error "LXC with ID ${CT_ID} already exists. Please choose a different ID."
fi
...
TEMPLATE_STORAGE=""
for storage_item in "${storage_options[@]}"; do
    content=$(pvesh get /storage/$storage_item --output-format yaml | grep "content:" | awk '{$1=""; print $0}')
    if [[ "$content" == *"vztmpl"* ]]; then
        TEMPLATE_STORAGE=$storage_item
        break
    fi
done
...
pct create $CT_ID $TEMPLATE_NAME --hostname $var_hostname --cores $var_cpu --memory $var_ram --rootfs ${STORAGE}:${var_disk} --net0 name=eth0,bridge=${BRIDGE},ip=dhcp --onboot 1 --unprivileged $var_unprivileged >/dev/null
if [ $? -ne 0 ]; then
...
for i in {1..30}; do
    IP=$(pct exec $CT_ID -- ip -4 a show dev eth0 | grep inet | awk '{print $2}' | cut -d/ -f1 2>/dev/null)
    if [ -n "$IP" ]; then
        break
    fi
    msg_info "Waiting for IP address... (${i}/30)"
    sleep 2
done
...
if [ -z "$IP" ]; then
    ...
    msg_error "Could not retrieve IP address for LXC ${CT_ID}. Aborting and cleaning up."
fi
